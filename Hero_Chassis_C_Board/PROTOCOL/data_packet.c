 ...
 */
/*裁判系统更改尝试*/
void dma_buffer_to_unpack_buffer(uart_dma_rxdata_t *dma_obj, uart_it_type_e it_type, uint16_t tmp_len)
{
	//双缓冲模式读取当前所用存储器0/1以及读取出要传输的数据项数目
	uint8_t  *pdata = dma_obj->buff[!DMA_GetCurrentMemoryTarget(dma_obj->dma_stream)];
 ...
 */
uint8_t packet_tx_buf[PROTOCAL_FRAME_MAX_SIZE];
uint8_t data_packet_pack(uint16_t cmd_id, uint8_t *p_data, uint16_t len, uint8_t sof)
{
//	static uint8_t a[119];
  
  uint16_t frame_length = HEADER_LEN + CMD_LEN + len + CRC_LEN;//发送那个数据包的总长度
  
  protocol_packet_pack(cmd_id, p_data, len, sof, packet_tx_buf);//将数据进行打包，组成一个完整的数据帧
//	memcpy(a,tx_buf,frame_length);//查看发送的完整数据，便于debug查看数据是否乱码
  
  /* use mutex operation */
	if (sof == DN_REG_ID)
		return fifo_s_puts(&judge_txdata_fifo, packet_tx_buf, frame_length);//将数据从tx_buf放到judge_txdata_fifo
	else
		return 0;
}
...
#endif
uint8_t send_packed_fifo_data(fifo_s_t *pfifo, uint8_t sof)
{
  ...
  if (fifo_count)
  {
    fifo_s_gets(pfifo, judge_txbuf, fifo_count);//将数据转移到tx_buf
    
    if (sof == DN_REG_ID)
//      usart_send_data(USART6, tx_buf, fifo_count);//通过寄存器发送数据
		return DMA_Judge_USART_Tx_Data(judge_txbuf, fifo_count);
    else
      return 0;
  }
  return 1;
