...
/* ��Ҫ�㹻������ݲ��ܵõ���Ч�����ǡ����ٶȼ���ƫУ׼��� */
#define 	CaliTimes 6000	//ȡ6k������,У׼6s
#define acc_CaliTimes 6000	//ȡ6k������,У׼6s	
...
{
	static u8 error = BMI088_NO_ERROR;
	...
	if (CALIBRATION)
	{
		GPIO_SetBits(GPIOH,LED_RED);	//У׼���̲��ܶ������������
		//��ҪУ׼���������imuУ׼��ʱ��Ҫ����ˮƽ��ֹ����
	...
	{
		BSP_FLASH_READ();
        BMI088.GyroOffset[0] = cali_param.gxoffset;
        BMI088.GyroOffset[1] = cali_param.gyoffset;
        BMI088.GyroOffset[2] = cali_param.gzoffset;
...
/* У׼��Ư,ֻ�л����ӵ�ʱ���������Ư�Ƶ�ʱ����ҪУ׼ */
static void Calibrate_MPU_Offset(IMU_Data_t *bmi088)
{
	static uint32_t startTime;
	uint8_t buf[8] = {0, 0, 0, 0, 0, 0};	// ��Ŵ��������ݵĻ�����
	int16_t bmi088_raw_temp;				// ��ʱ��Ŵ�������ԭʼ����
	float gyroMax[3], gyroMin[3];			// �����ǵ����ֵ����Сֵ
	float gNormTemp, gNormMax, gNormMin;	// ������ٶ�������ģֵ
	float gyroDiff[3], gNormDiff;			// ���ڼ�⻷���ȶ���
	int16_t caliCount = 0;					// ��¼У׼���ԵĴ���

	startTime = HAL_GetTick();		// ��¼У׼��ʼ��ʱ��
	do
	{
		/* ��У׼ʱ�䳬��10��(10000ms)����ʹ��Ĭ��ֵ�˳�У׼ */
		if (HAL_GetTick() - startTime > 10000)
		{
			// У׼��ʱ
			bmi088->GyroOffset[0] = GxOFFSET;// �趨Ĭ�ϵ�X��ƫֵ
			bmi088->GyroOffset[1] = GyOFFSET;// �趨Ĭ�ϵ�Y��ƫֵ
			bmi088->GyroOffset[2] = GzOFFSET;// �趨Ĭ�ϵ�Z��ƫֵ
			bmi088->gNorm = gNORM;			// �趨Ĭ�ϵ��������ٶ�ģֵ��9.84484291f
			bmi088->TempWhenCali = 40;		// �趨Ĭ�ϵ�У׼�¶�
			break;							
		}

		DWT_Delay(0.005);					// �����ӳ٣�����������
		bmi088->gNorm = 0;					// ����ģֵ����
		bmi088->GyroOffset[0] = 0;			// ��������ƫ��ʼֵ����
		bmi088->GyroOffset[1] = 0;			// ��������ƫ��ʼֵ����	
		bmi088->GyroOffset[2] = 0;			// ��������ƫ��ʼֵ����

		/* ����CaliTimes�β�������ȡ�����Ǻͼ��ٶȼƵ���ƫ */
		for (uint16_t i = 0; i < CaliTimes; i++)
		{
			/* ��ȡ���ٶȼ����� */
			BMI088_accel_read_muli_reg(BMI088_ACCEL_XOUT_L, buf, 6);
			bmi088_raw_temp = (int16_t)((buf[1]) << 8) | buf[0];	// X����ٶ�
			bmi088->Accel[0] = bmi088_raw_temp * BMI088_ACCEL_SEN;
			bmi088_raw_temp = (int16_t)((buf[3]) << 8) | buf[2];	// Y����ٶ�
			bmi088->Accel[1] = bmi088_raw_temp * BMI088_ACCEL_SEN;
			bmi088_raw_temp = (int16_t)((buf[5]) << 8) | buf[4];	// Z����ٶ�
			...
			bmi088->gNorm += gNormTemp;								// �ۼ�ģֵ
			
			/* ��ȡ���������� */
			BMI088_gyro_read_muli_reg(BMI088_GYRO_CHIP_ID, buf, 8);
			if (buf[0] == BMI088_GYRO_CHIP_ID_VALUE)	// ȷ����ȡ������ȷ��������оƬ	
			{
				bmi088_raw_temp = (int16_t)((buf[3]) << 8) | buf[2];
				bmi088->Gyro[0] = bmi088_raw_temp * BMI088_GYRO_SEN;// X����ٶ�	
				bmi088->GyroOffset[0] += bmi088->Gyro[0];			//�ۻ���Ʈֵ
				
				bmi088_raw_temp = (int16_t)((buf[5]) << 8) | buf[4];
				bmi088->Gyro[1] = bmi088_raw_temp * BMI088_GYRO_SEN;// Y����ٶ�
				bmi088->GyroOffset[1] += bmi088->Gyro[1];			//�ۻ���Ʈֵ
				
				bmi088_raw_temp = (int16_t)((buf[7]) << 8) | buf[6];
				bmi088->Gyro[2] = bmi088_raw_temp * BMI088_GYRO_SEN;// Z����ٶ�
				bmi088->GyroOffset[2] += bmi088->Gyro[2];			//�ۻ���Ʈֵ
			}
	...
	for (uint16_t i = 0; i < acc_CaliTimes; i++)
	{
		...
		bmi088->Accel[0] = bmi088_raw_temp * BMI088_ACCEL_SEN * bmi088->AccelScale;			// ���� X ����ٶ�
		bmi088->AccelOffset[0] = bmi088->AccelOffset[0] * 0.8f + bmi088->Accel[0] * 0.2f;	// ��ͨ�˲����� X ��ƫ����

		bmi088_raw_temp = (int16_t)((buf[3]) << 8) | buf[2];
		bmi088->Accel[1] = bmi088_raw_temp * BMI088_ACCEL_SEN * bmi088->AccelScale;			// ���� Y ����ٶ�
		bmi088->AccelOffset[1] = bmi088->AccelOffset[1] * 0.8f + bmi088->Accel[1] * 0.2f;	// ��ͨ�˲����� Y ��ƫ�������ٶ�һ�׵�ͨ�˲�

		bmi088_raw_temp = (int16_t)((buf[5]) << 8) | buf[4];
		bmi088->Accel[2] = bmi088_raw_temp * BMI088_ACCEL_SEN * bmi088->AccelScale;			// ���� Z ����ٶ�
		bmi088->AccelOffset[2] = bmi088->AccelOffset[2] * 0.8f + bmi088->Accel[2] * 0.2f;	// ��ͨ�˲����� Z ��ƫ�������ٶ�һ�׵�ͨ�˲�
    ...
    cali_param.gxoffset = bmi088->GyroOffset[0];
    cali_param.gyoffset = bmi088->GyroOffset[1];
    cali_param.gzoffset = bmi088->GyroOffset[2];

    cali_param.gnormal = bmi088->gNorm;
    cali_param.accelscale = bmi088->AccelScale;
