...
void image_unpack_fifo_data(image_unpack_data_t *p_obj, uint8_t sof1, uint8_t sof2)
{
	uint8_t byte = 0;
	
	if(fifo_used_count(p_obj->data_fifo) < 21)
		return;	
	...
	while (fifo_used_count(p_obj->data_fifo))
	{
		...
		case STEP_HEADER_SOF2:
		{
			if (byte == sof2)
			{
				p_obj->unpack_step = STEP_IMAGE_CRC16;
				p_obj->protocol_packet[p_obj->index++] = byte;
			}
			else
			{
		...
		case STEP_IMAGE_CRC16:
		{
			if (p_obj->index < IMAGE_BUFLEN)
			{
				p_obj->protocol_packet[p_obj->index++] = byte;
			}
			if (p_obj->index >= IMAGE_BUFLEN)
			{
				...
				if (verify_crc16_check_sum(p_obj->protocol_packet, IMAGE_BUFLEN))
				{
					if(rc.remote_flag == Old_Remote_enable)
					{
						if(global_err.list[REMOTE_CTRL_OFFLINE].err_exist)
							rc.remote_flag = New_Remote_enable;
							
					}
					else //新遥控接收数据
					{
						transfer_image_data_handler(&rc, p_obj->protocol_packet);
